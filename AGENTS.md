...
- Validate and bound external or durable inputs at their owning boundary.
- Keep DRY and avoid restating anything that is already self-explanatory. Give each current contract—not its change history—one authoritative home and link to it; **update contracts before implementation**.
...
- Select verification by changed surface using the closest behavior tests and
