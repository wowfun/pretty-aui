...
class DefaultChatController {
    ...
    send(input) {
        ...
        if (this.#sessionMutations.has(session.sessionId)) {
            throw new PrettyAuiError("SESSION_BUSY", `Wait for the current operation on session '${session.sessionId}'`, { protocol: this.#driver?.version, phase: "prompt" });
        }
        ...
        if (session.phase !== "idle" ||
            ...
            throw new PrettyAuiError("SESSION_BUSY", `Session '${session.sessionId}' is not ready for another prompt`, { protocol: this.#driver?.version, phase: "prompt" });
        }
        const blocks = normalizeInput(input);
        if (blocks.some((block) => block._meta !== null &&
            block._meta !== undefined &&
            Object.hasOwn(block._meta, "pretty-aui/context"))) {
            throw new PrettyAuiError("INVALID_CONFIGURATION", "Prompt input cannot use the reserved pretty-aui/context metadata key", { phase: "prompt" });
        }
        if (!blocks.length ||
    ...
    async addContext() {
        ...
        if (!provider.add) {
            throw new PrettyAuiError("METHOD_NOT_AVAILABLE", "The context provider does not support adding context", { phase: "context/add" });
        }
    ...
    async removeContext(id) {
        ...
        if (!provider.remove) {
            throw new PrettyAuiError("METHOD_NOT_AVAILABLE", "The context provider does not support removing context", { phase: "context/remove" });
        }
        if (!this.#contextSelection.some((item) => item.id === id)) {
            throw new PrettyAuiError("INVALID_CONFIGURATION", `Unknown context selection '${id}'`, { phase: "context/remove" });
        }
    ...
    async reconnect() {
        ...
        await this.#withNewSessionOperation("connection/reconnect", () => this.#connect(false));
    }
    async newSession() {
    ...
    async openSession(sessionId) {
        ...
        if (loaded) {
            if (request === this.#selectionRequest)
                this.#selectSession(loaded);
            return;
        }
        this.#assertSessionCapacity();
        await this.#withTargetSessionOperation(sessionId, "session/open", async () => this.#openSession(this.#requireDriver(), sessionId, [], request));
    }
    async openChildSession(sessionId) {
        ...
        this.#assertSessionCapacity();
        await this.#withTargetSessionOperation(sessionId, "session/open-child", async () => this.#openSession(this.#requireDriver(), sessionId, trail, request));
    }
    async openAncestorSession(sessionId) {
        ...
        this.#assertSessionCapacity();
        await this.#withTargetSessionOperation(sessionId, "session/open-ancestor", async () => this.#openSession(this.#requireDriver(), sessionId, trail, request));
    }
    async closeSession(sessionId) {
        const target = this.#requireSession(sessionId);
        this.#assertSessionIdle(target, "session/close");
        await this.#withTargetSessionOperation(target.sessionId, "session/close", async () => {
            const driver = this.#requireDriver();
            ...
            if (this.#snapshot.sessionId === target.sessionId) {
                const fallback = [...this.#sessions.values()].sort((left, right) => right.lastSelected - left.lastSelected)[0];
                if (fallback)
                    this.#selectSession(fallback);
                else
                    this.#clearSelection();
            }
            else {
    ...
    async deleteSession(sessionId) {
        ...
        const loaded = this.#sessions.get(sessionId);
        if (loaded)
            this.#assertSessionIdle(loaded, "session/delete");
        await this.#withTargetSessionOperation(sessionId, "session/delete", async () => {
            const driver = this.#requireDriver();
    ...
    async setConfigOption(id, value) {
        ...
        await this.#withTargetSessionOperation(session.sessionId, "session/set-config", async () => {
            const driver = this.#requireDriver();
            const returned = await driver.setConfigOption(session.sessionId, id, value);
            this.#assertDriverCurrent(driver);
    ...
    async authenticate(methodId) {
        if (this.#options.allowAuthentication === false) {
            throw new PrettyAuiError("AUTHENTICATION_DISABLED", "Agent authentication is disabled by the host", { phase: "auth/login" });
        }
    ...
    async destroy() {
        ...
        const interrupted = new PrettyAuiError("TURN_INTERRUPTED", "Chat was destroyed before the turn completed", { phase: "destroy", retryable: false });
    ...
    async #connectInner(createSession) {
        ...
        try {
            ...
            this.#snapshot = cleanSnapshot({
                ...
                authMethods: this.#options.allowAuthentication === false
                    ? []
            ...
            if (createSession) {
                ...
                if (initial.type === "open") {
                    this.#assertSessionCapacity();
                    await this.#openSession(driver, initial.sessionId, [], ++this.#selectionRequest);
                    this.#connectionPhase = "ready";
                    this.#publish();
                    return;
                }
            ...
            const ordered = [...previousSessions].sort((left, right) => {
                if (left.sessionId === previousSessionId)
                    return -1;
                if (right.sessionId === previousSessionId)
                    return 1;
            ...
            for (const oldSession of ordered) {
                try {
                    const history = driver.initialized.capabilities.resumeSession
                        ? "none"
                        : "all";
                    const timeline = history === "none" ? oldSession.timeline : new TimelineStore();
                    const staging = {
            ...
            if (error instanceof PrettyAuiError &&
                ...
                if (this.#options.allowAuthentication === false) {
                    const disabled = new PrettyAuiError("AUTHENTICATION_DISABLED", "The agent requires authentication disabled by the host", { cause: error, protocol: driver?.version, phase: "session/new" });
                    this.#fail(disabled);
                    throw disabled;
                }
                this.#connectionPhase = "auth_required";
    ...
    async #runTurn(turn, input) {
        try {
            ...
            const contextItems = await this.#resolveContext(session.sessionId, input, turn.contextSelection, turn.abort.signal);
            ...
            if (!wireMessageWithinBudget({
                ...
                throw new PrettyAuiError("INVALID_CONFIGURATION", "The prepared ACP prompt exceeds the 2 MiB wire-message limit", { protocol: driver.version, phase: "prompt" });
            }
            throwIfAborted(turn.abort.signal);
            turn.submitted = true;
    ...
    async #resolveContext(sessionId, input, selection, signal) {
        try {
            const source = this.#options.context;
            if (!source)
                return [];
            const items = isContextProvider(source)
                ? await abortable(source.resolve({
                    sessionId,
                    input,
                    selection,
                    ...(this.#snapshot.protocolVersion
                        ...
                        : {}),
                    capabilities: this.#requireDriver().initialized.promptCapabilities,
                    signal,
                }), signal)
                : source;
            if (isContextProvider(source) &&
                (items.length !== selection.length ||
                    items.some((item, index) => item.id !== selection[index]?.id))) {
                throw new Error("Resolved context IDs must match the frozen selection order");
            }
            const ids = new Set();
            if (items.length > 64)
            ...
            for (const item of items) {
                ...
                if (typeof item.label !== "string" ||
                    ...
                    throw new Error("Context item labels must be non-empty bounded strings");
                }
                if (!Array.isArray(item.content) || !item.content.length) {
                    throw new Error("Context items must contain at least one content block");
                }
                ids.add(item.id);
            }
            ...
            const driver = this.#requireDriver();
            validatePrompt(canonical.flatMap((item) => item.content), driver.initialized.promptCapabilities, driver.version);
            return canonical;
        }
        catch (error) {
            if (signal.aborted)
    ...
    async #openSession(driver, sessionId, sessionTrail, selectionRequest) {
        const staging = {
            sessionId,
    ...
    async #applyNewSessionMode(driver, session) {
        ...
        try {
            const returned = await driver.setConfigOption(session.sessionId, mode.id, preferred);
            this.#assertDriverCurrent(driver);
            ...
            this.#emit({
                ...
                message: "The fixed new-session mode could not be applied; using the Agent default",
            });
            return session;
        }
    }
        ...
        if (effect.state &&
            ...
            if (effect.state === "idle") {
                ...
                if (effect.stopReason)
                    session.stopReason = effect.stopReason;
                summaryChanged = true;
            }
        }
        if (effect.commands)
            session.commands = effect.commands;
    ...
    #removeInteraction(id, sessionId) {
        if (sessionId === undefined) {
            this.#connectionInteractions = this.#connectionInteractions.filter((interaction) => interaction.id !== id);
            this.#publish();
            return;
        }
        const session = this.#sessions.get(sessionId);
        if (!session)
            return;
        session.interactions = session.interactions.filter((interaction) => interaction.id !== id);
    ...
    #assertSessionIdle(session, phase) {
        if (session.activeTurn || session.interactions.length) {
            throw new PrettyAuiError("SESSION_BUSY", `Finish session '${session.sessionId}' before changing it`, { phase });
        }
    }
    #assertConnectionReplaceable() {
        if (this.#destroyed)
            throw connectionClosedError();
        if (this.#connectionInteractions.length ||
            [...this.#sessions.values()].some((session) => session.activeTurn || session.interactions.length)) {
            throw new PrettyAuiError("SESSION_BUSY", "Finish all turns and interactions before replacing the connection", { phase: "connection/reconnect" });
        }
    }
    #assertSessionCapacity() {
        const reservations = [...this.#staging.keys()].filter((sessionId) => !this.#sessions.has(sessionId)).length;
        if (this.#sessions.size + reservations < 16)
            return;
        throw new PrettyAuiError("SESSION_LIMIT", "Close a loaded session before opening another one", { phase: "session" });
    }
    #mapAuthenticationFailure(error, phase) {
        if (!(error instanceof PrettyAuiError) ||
            error.code !== "AUTHENTICATION_REQUIRED" ||
        ...
        if (this.#options.allowAuthentication === false) {
            const disabled = new PrettyAuiError("AUTHENTICATION_DISABLED", "The agent requires authentication disabled by the host", { cause: error, protocol: this.#driver.version, phase });
            this.#fail(disabled);
            return disabled;
        }
        this.#connectionPhase = "auth_required";
        this.#connectionError = undefined;
        this.#publish();
        return new PrettyAuiError("AUTHENTICATION_REQUIRED", "Authentication is required before a session can be created", { cause: error, protocol: this.#driver.version, phase });
    }
    #requireDriver() {
        if (this.#destroyed)
            throw connectionClosedError();
    ...
    #requireSession(sessionId = this.#snapshot.sessionId) {
        if (!sessionId) {
        ...
        const session = this.#sessions.get(sessionId);
        if (!session) {
            throw new PrettyAuiError("SESSION_NOT_READY", `Session '${sessionId}' is not loaded`, { phase: "session" });
        }
        return session;
    }
    #createSession(session, timeline = new TimelineStore(), staging, instanceId = `session-instance-${++this.#sessionInstanceCounter}`) {
        const catalogTitle = this.#snapshot.sessions?.sessions.find((candidate) => candidate.sessionId === session.sessionId)?.title;
    ...
    #contextProvider(operation) {
        ...
        if (!isContextProvider(provider)) {
            throw new PrettyAuiError("METHOD_NOT_AVAILABLE", "The configured context is not mutable", { phase: `context/${operation}` });
        }
        return provider;
    }
    async #mutateContext(phase, operation) {
        if (this.#destroyed)
            throw connectionClosedError();
        if (this.#contextMutation) {
            throw new PrettyAuiError("SESSION_BUSY", "Wait for the current context change to finish", { phase });
        }
        ...
        try {
            ...
            if (this.#destroyed)
                throw connectionClosedError();
            throw new PrettyAuiError("CONTEXT_FAILED", "Context selection could not be changed", { cause: error, phase, retryable: true });
        }
        finally {
    ...
    #publish() {
        if (this.#destroyed && this.#connectionPhase !== "closed")
            return;
        const selected = this.#selectedSession();
        const connectionPhase = this.#connectionPhase;
        const phase = connectionPhase === "ready"
            ? (selected?.phase ?? "idle")
            : connectionPhase;
        const error = connectionPhase === "error" ? this.#connectionError : selected?.error;
        this.#snapshot = cleanSnapshot({
            protocolVersion: this.#snapshot.protocolVersion,
            ...
            while (this.#sessionTitleRefreshRequested && !this.#destroyed) {
                ...
                try {
                    ...
                    this.#emit({
                        ...
                        message: "The session title could not be refreshed from the Agent catalog",
                    });
                }
            }
        })
            .finally(() => {
            if (this.#sessionTitleRefresh !== operation)
                return;
    ...
    async #withNewSessionOperation(phase, operation) {
        if (this.#destroyed)
            throw connectionClosedError();
        if (this.#newSessionMutation) {
            throw new PrettyAuiError("SESSION_BUSY", "Wait for the current connection-level session operation to finish", { protocol: this.#driver?.version, phase });
        }
        if (this.#sessionMutations.size) {
            throw new PrettyAuiError("SESSION_BUSY", "Wait for target-session operations to finish", { protocol: this.#driver?.version, phase });
        }
    ...
    async #withTargetSessionOperation(sessionId, phase, operation) {
        if (this.#destroyed)
            throw connectionClosedError();
        if (this.#newSessionMutation) {
            throw new PrettyAuiError("SESSION_BUSY", "Wait for the current connection-level session operation to finish", { protocol: this.#driver?.version, phase });
        ...
        if (this.#sessionMutations.has(sessionId)) {
            throw new PrettyAuiError("SESSION_BUSY", `Wait for the current operation on session '${sessionId}'`, { protocol: this.#driver?.version, phase });
        }
        const mutation = Symbol(phase);
...
function readContextSelection(source) {
    if (!source)
    ...
    if (!Array.isArray(values)) {
        throw new PrettyAuiError("INVALID_CONFIGURATION", "Context selection must be an array", { phase: "context/selection" });
    }
    if (values.length > 64) {
        throw new PrettyAuiError("INVALID_CONFIGURATION", "Context is limited to 64 selected items", { phase: "context/selection" });
    }
    const ids = new Set();
    const items = values.map((value) => {
        if (!isRecord(value) ||
            ...
            throw new PrettyAuiError("INVALID_CONFIGURATION", "Context selection IDs must be non-empty bounded strings", { phase: "context/selection" });
        }
        if (ids.has(value.id)) {
            throw new PrettyAuiError("INVALID_CONFIGURATION", `Context selection IDs must be unique: '${value.id}'`, { phase: "context/selection" });
        }
        if (typeof value.label !== "string" ||
            !value.label.trim() ||
            value.label.length > 16 * 1024) {
            throw new PrettyAuiError("INVALID_CONFIGURATION", "Context selection labels must be non-empty bounded strings", { phase: "context/selection" });
        }
        ids.add(value.id);
...
function canonicalContextBlock(value) {
    ...
    switch (value.type) {
        ...
        case "audio":
            if (typeof value.data !== "string" || typeof value.mimeType !== "string")
                throw new Error(`Context ${value.type} blocks require data and mimeType`);
            return {
                ...base,
                type: value.type,
                data: value.data,
...
function canonicalContentBase(value) {
    ...
    const annotations = isRecord(value.annotations)
        ? {
            ...(Array.isArray(value.annotations.audience)
                ? {
                    audience: value.annotations.audience.filter((item) => item === "user" || item === "assistant"),
                }
                : {}),
...
function contextFailureMessage(error) {
    ...
    if (detail.length > MAX_CONTEXT_FAILURE_DETAIL_LENGTH)
        bounded += "…";
    return `${summary}: ${bounded}`;
}
function findModeOption(options) {
    return (options.find((option) => option.category === "mode" && option.type === "select") ??
        options.find((option) => option.id === "mode" && option.type === "select"));
}
function updateSessionConfigOption(session, option, value, returned) {
    return {
        ...session,
        configOptions: returned.length
            ? returned
            : session.configOptions.map((candidate) => candidate.id === option.id
