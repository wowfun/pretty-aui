...
const OPENING_PATTERN = new RegExp(`(?:[\\t ]*\\r?\\n){0,2}[\\t ]*<${USER_MESSAGE_TAG_PREFIX}(${TOKEN_SOURCE})>[\\t ]*\\r?\\n`, "g");
const CLOSING_PATTERN = new RegExp(`(?:[\\t ]*\\r?\\n)?[\\t ]*</${USER_MESSAGE_TAG_PREFIX}(${TOKEN_SOURCE})>`, "g");
const MARKER_SIGNAL_PATTERN = new RegExp(`${USER_MESSAGE_TAG_PREFIX}${TOKEN_SOURCE}`, "g");
let fallbackTokenCounter = 0;
export function createUserMessageEnvelopeToken() {
...
export function envelopeUserPrompt(input, token) {
    if (!token ||
...
export function restoreUserPrompt(content) {
    const spans = [];
    ...
    for (const block of content) {
        ...
        for (let signal = MARKER_SIGNAL_PATTERN.exec(searchable); signal;) {
            sawMarker = true;
            lastMarkerSignalEnd = Math.max(lastMarkerSignalEnd, searchableStart + signal.index + signal[0].length);
    ...
    const restored = sliceContent(spans, complete.opening.end, complete.closing.start);
    const context = groupRestoredContext(sliceContent(spans, 0, complete.opening.start), complete.opening.token);
    return { status: "restored", content: restored, context };
}
function sliceContent(spans, start, end) {
    const sliced = [];
    for (const span of spans) {
        if (span.block.type === "text" && typeof span.block.text === "string") {
            ...
            if (sliceStart < sliceEnd) {
                const text = span.block.text.slice(sliceStart - span.start, sliceEnd - span.start);
...
function groupRestoredContext(content, token) {
    const groups = [];
    for (const block of content) {
...
function contextMetadata(block) {
    if (!isRecord(block._meta))
        return undefined;
...
function recoveredContextLabel(content) {
    for (const block of content) {
        if (block.type === "text" && typeof block.text === "string") {
            const line = block.text
                ...
                .find(Boolean);
            if (line)
                return line.slice(0, MAX_RECOVERED_LABEL_LENGTH);
        }
