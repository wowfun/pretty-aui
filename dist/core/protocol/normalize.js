...
export class TimelineStore {
    ...
    #appendMessage(role, messageId, content, protocol) {
        ...
        if (role === "user" && this.#pendingUserId) {
            ...
            const existing = this.#activities.find((item) => item.type === "message" && item.id === pendingId);
    ...
    #appendActivity(activity) {
        ...
        if (this.#activities.length <= MAX_ACTIVITIES)
            return;
        const removed = this.#activities.slice(0, this.#activities.length - MAX_ACTIVITIES);
        this.#activities = this.#activities.slice(-MAX_ACTIVITIES);
        for (const candidate of removed) {
            if (candidate.id === this.#pendingUserId)
                this.#pendingUserId = undefined;
